...
class Menu extends Component {
    ...
    render() {
        ...
        const menu_content = menuItems ? menuItems.map((text) => {
                ...
                return (<Grid item xs={12} sm={12} md={6} lg={4} className={classes.menuSubContent} key={text.target}
                              component={Link} to={`/${text.target.toLowerCase()}`}
        ...
        return (<div className='menu-list-item'>
            <Grid container className={classes.menuContent}>
                <Grid className={classes.menuCards} item xs={12} sm={12} md={6} lg={4}
                      style={{borderRight: "1px solid #e3e3e3", borderBottom: "1px solid #e3e3e3",}}><SvCards
                    cards={this.state.uzCards}
                    lang={this.props.menuItems ? this.props.menuItems.language : "ru"}/></Grid>
                {this.state.uzCards.length ? this.state.valCards.length ?
                    <Grid className={classes.menuCards} item xs={12} sm={12} md={6} lg={4}
                          style={{borderRight: "1px solid #e3e3e3", borderBottom: "1px solid #e3e3e3",}}><TetCards
                        cards={this.state.valCards} lang={this.props.menuItems ? this.props.menuItems.language : "ru"}/></Grid> : "" : ""}
                {this.state.depositsList !== "" ? this.state.depositsList.depositsJson && this.state.depositsList.depositsJson.deposits.length ?
                    <Grid className={classes.menuCards} item xs={12} sm={12} md={6} lg={4}
                          style={{borderRight: "1px solid #e3e3e3", borderBottom: "1px solid #e3e3e3"}}><Deposits
                        cards={this.state.valCards} deposits={this.state.depositsList}
                        depositsAdded={this.props.menuItems.depositsAdded}
                        lang={this.props.menuItems ? this.props.menuItems.language : "ru"}/></Grid> : "" : ""}
            </Grid>
            <Grid container className={classes.menuContent}>
